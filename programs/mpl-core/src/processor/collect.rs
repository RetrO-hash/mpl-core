...
fn collect_from_account(
    ...
    let dest1_starting_lamports = dest1_info.lamports();
    let new_dest1_lamports = dest1_starting_lamports
        .checked_add(split_fee_amount)
        .ok_or(MplCoreError::NumericalOverflowError)?;
    **dest1_info.try_borrow_mut_lamports()? = new_dest1_lamports;

    let dest2_starting_lamports = dest2_info.lamports();
    let dest2_fee_amount = fee_amount
        .checked_sub(split_fee_amount)
        .ok_or(MplCoreError::NumericalOverflowError)?;
    let new_dest2_lamports = dest2_starting_lamports
        .checked_add(dest2_fee_amount)
        .ok_or(MplCoreError::NumericalOverflowError)?;
    **dest2_info.try_borrow_mut_lamports()? = new_dest2_lamports;
