...
fn collect_from_account(
    ...
    let dest1_lamports = dest1_info.lamports();
    let new_dest1_lamports = dest1_lamports
        .checked_add(split_fee_amount)
        .ok_or(MplCoreError::NumericalOverflowError)?;
    **dest1_info.try_borrow_mut_lamports()? = new_dest1_lamports;

    let dest2_lamports = dest2_info.lamports();
    let new_dest2_lamports = dest2_lamports
        .checked_add(
            fee_amount
