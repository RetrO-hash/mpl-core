...
pub(crate) fn close_program_account<'a>(
    ...
    **account_to_close_info.try_borrow_mut_lamports()? = current_account_lamports
...
pub(crate) fn resize_or_reallocate_account<'a>(
    ...
    if new_minimum_balance >= current_minimum_balance {
        ...
        **funding_account.try_borrow_mut_lamports()? = funding_lamports
            .checked_add(lamports_diff)
            .ok_or(MplCoreError::NumericalOverflowError)?;

        let target_lamports = target_account.lamports();
        **target_account.try_borrow_mut_lamports()? = target_lamports
