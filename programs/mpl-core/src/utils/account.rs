...
pub(crate) fn close_program_account<'a>(
    ...
    **funds_dest_account_info.try_borrow_mut_lamports()? = dest_starting_lamports
    ...
    let src_starting_lamports = account_to_close_info.lamports();
    **account_to_close_info.try_borrow_mut_lamports()? = src_starting_lamports
        .checked_sub(amount_to_return)
        .ok_or(MplCoreError::NumericalOverflowError)?;

    account_to_close_info.realloc(1, false)?;
    let uninitialized_key = Key::Uninitialized
        .to_u8()
        .ok_or(MplCoreError::NumericalOverflowError)?;
    account_to_close_info.try_borrow_mut_data()?[0] = uninitialized_key;
...
pub(crate) fn resize_or_reallocate_account<'a>(
    ...
    if new_minimum_balance >= current_minimum_balance {
        ...
        **funding_account.try_borrow_mut_lamports()? = funding_starting_lamports
            .checked_add(lamports_diff)
            .ok_or(MplCoreError::NumericalOverflowError)?;

        let target_starting_lamports = target_account.lamports();
        **target_account.try_borrow_mut_lamports()? = target_starting_lamports
