...
pub(crate) fn close_program_account<'a>(
    ...
    let new_dest_lamports = dest_starting_lamports.checked_add(amount_to_return).ok_or(MplCoreError::NumericalOverflowError)?;
    **funds_dest_account_info.try_borrow_mut_lamports()? = new_dest_lamports;

    let new_source_lamports = account_to_close_info.lamports().checked_sub(amount_to_return).ok_or(MplCoreError::NumericalOverflowError)?;
    **account_to_close_info.try_borrow_mut_lamports()? = new_source_lamports;
...
pub(crate) fn resize_or_reallocate_account<'a>(
    ...
    if new_minimum_balance >= current_minimum_balance {
        ...
        let new_funding_lamports = funding_account.lamports().checked_add(lamports_diff).ok_or(MplCoreError::NumericalOverflowError)?;
        **funding_account.try_borrow_mut_lamports()? = new_funding_lamports;
        let new_target_lamports = target_account.lamports().checked_sub(lamports_diff).ok_or(MplCoreError::NumericalOverflowError)?;
