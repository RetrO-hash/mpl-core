...
pub(crate) fn close_program_account<'a>(
    ...
    **funds_dest_account_info.try_borrow_mut_lamports()? = dest_starting_lamports
    ...
    let old_lamports = account_to_close_info.lamports();
    **account_to_close_info.try_borrow_mut_lamports()? = old_lamports
...
pub(crate) fn resize_or_reallocate_account<'a>(
    ...
    if new_minimum_balance >= current_minimum_balance {
        ...
        let old_funding = funding_account.lamports();
        **funding_account.try_borrow_mut_lamports()? = old_funding
            .checked_add(lamports_diff)
            .ok_or(MplCoreError::NumericalOverflowError)?;

        let old_target = target_account.lamports();
        **target_account.try_borrow_mut_lamports()? = old_target
