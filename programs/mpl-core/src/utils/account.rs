...
pub(crate) fn close_program_account<'a>(
    ...
    let new_lamports = dest_starting_lamports
        .checked_add(amount_to_return)
        .ok_or(MplCoreError::NumericalOverflowError)?;
    **funds_dest_account_info.try_borrow_mut_lamports()? = new_lamports;

    let current_lamports = account_to_close_info.lamports();
    let new_account_lamports = current_lamports
        .checked_sub(amount_to_return)
        .ok_or(MplCoreError::NumericalOverflowError)?;
    **account_to_close_info.try_borrow_mut_lamports()? = new_account_lamports;

    account_to_close_info.realloc(1, false)?;
    account_to_close_info.try_borrow_mut_data()?[0] = Key::Uninitialized.to_u8().ok_or(MplCoreError::SerializationError)?;
...
pub(crate) fn resize_or_reallocate_account<'a>(
    ...
    if new_minimum_balance >= current_minimum_balance {
        ...
        let funding_lamports = funding_account.lamports();
        let new_funding_lamports = funding_lamports
        ...
        **funding_account.try_borrow_mut_lamports()? = new_funding_lamports;
        ...
        let new_target_lamports = target_lamports
