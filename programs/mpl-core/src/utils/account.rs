...
pub(crate) fn close_program_account<'a>(
    ...
    **account_to_close_info.try_borrow_mut_lamports()? = account_starting_lamports
        .checked_sub(amount_to_return)
        .ok_or(MplCoreError::NumericalOverflowError)?;

    account_to_close_info.realloc(1, false)?;
    let uninitialized_key = Key::Uninitialized.to_u8().ok_or(MplCoreError::NumericalOverflowError)?;
