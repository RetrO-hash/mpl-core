...
pub(crate) fn close_program_account<'a>(
    ...
    **account_to_close_info.try_borrow_mut_lamports()? = current_lamports
...
pub(crate) fn resize_or_reallocate_account<'a>(
    ...
    if new_minimum_balance >= current_minimum_balance {
        ...
        let current_funding_lamports = funding_account.lamports();
        **funding_account.try_borrow_mut_lamports()? = current_funding_lamports
            .checked_add(lamports_diff)
            .ok_or(MplCoreError::NumericalOverflowError)?;

        let current_target_lamports = target_account.lamports();
        **target_account.try_borrow_mut_lamports()? = current_target_lamports
