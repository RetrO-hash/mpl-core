...
pub(crate) fn close_program_account<'a>(
    ...
    **funds_dest_account_info.try_borrow_mut_lamports()? = dest_new_lamports;
    ...
    let source_lamports = account_to_close_info.lamports();
    let source_new_lamports = source_lamports
        .checked_sub(amount_to_return)
        .ok_or(MplCoreError::NumericalOverflowError)?;
    **account_to_close_info.try_borrow_mut_lamports()? = source_new_lamports;
...
pub(crate) fn resize_or_reallocate_account<'a>(
    ...
    if new_minimum_balance >= current_minimum_balance {
        ...
        let target_new_lamports = target_lamports
        ...
        **target_account.try_borrow_mut_lamports()? = target_new_lamports;
        ...
        let funding_new_lamports = funding_lamports
