...
pub(crate) fn close_program_account<'a>(
    ...
    **funds_dest_account_info.try_borrow_mut_lamports()? = new_dest_lamports;
    ...
    let account_to_close_lamports = account_to_close_info.lamports();
    let new_account_to_close_lamports = account_to_close_lamports
        .checked_sub(amount_to_return)
        .ok_or(MplCoreError::NumericalOverflowError)?;

    **account_to_close_info.try_borrow_mut_lamports()? = new_account_to_close_lamports;

    account_to_close_info.realloc(1, false)?;
    account_to_close_info.try_borrow_mut_data()?[0] = Key::Uninitialized
        .to_u8()
...
pub(crate) fn resize_or_reallocate_account<'a>(
    ...
    if new_minimum_balance >= current_minimum_balance {
        ...
        let new_funding_account_lamports = funding_account_lamports
            .checked_add(lamports_diff)
            .ok_or(MplCoreError::NumericalOverflowError)?;
        **funding_account.try_borrow_mut_lamports()? = new_funding_account_lamports;

        let target_account_lamports = target_account.lamports();
        let new_target_account_lamports = target_account_lamports
            .checked_sub(lamports_diff)
            .ok_or(MplCoreError::NumericalOverflowError)?;
        **target_account.try_borrow_mut_lamports()? = new_target_account_lamports;
    }
